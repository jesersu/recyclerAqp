...
export async function getMovieDetails(id) {
    const MOVIE_DETAILS = `https://api.themoviedb.org/3/movie/${id}`;
    ...
    const { original_title, overview, popularity, status, production_companies } = json;
    const { logo_path } = production_companies[0];
    const img = `https://image.tmdb.org/t/p/w185${logo_path}`;
